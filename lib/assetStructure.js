...
export const buildAssetStructure = () => {
  ...
  const assetSubCategoryRowsById = new Map();

  assetSubCategoryRows.forEach((row) => {
    const id = Number(row?.id);
    if (Number.isInteger(id) && id > 0) {
      assetSubCategoryRowsById.set(id, row);
    }
  });
  ...
  const ensureAssetSubCategory = (assetSubCategoryId) => {
    ...
    const row = assetSubCategoryRowsById.get(assetSubCategoryId) ?? {};
    const title =
      normaliseText(row.title || row.name) || `AssetUnterKategorie ${assetSubCategoryId}`;
    ...
    const slugBase = row.slug || title;
    entry.slug = slugify(slugBase || `AssetUnterKategorie ${assetSubCategoryId}`);
  ...
  assetSubCategoryRowsById.forEach((row, assetSubCategoryId) => {
    const assetSubCategory = ensureAssetSubCategory(assetSubCategoryId);
    if (!assetSubCategory) {
      return;
    }
