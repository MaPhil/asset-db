...
import { buildAssetRowValues } from './assetRowUtils.js';
...
export function executeGroupAssetSelector(groupSlug, selectorId) {
  const group = requireGroup(groupSlug);
  const selector = findSelectorRow(group.slug, selectorId);
  const snapshot = buildSnapshot();
  ...
  const assetIds = Array.from(matchedAssets).sort();
  const now = new Date().toISOString();
  store.update('groups', group.slug, {
    assets: assetIds,
    updated_at: now
  });
  ...
  return {
    selector: {
      id: selector.id,
      name: selector.name || `Selector ${selector.id}`,
...
export function calculateGroupSelectors() {
  const snapshot = buildSnapshot();
  const rows = Array.isArray(snapshot.rows) ? snapshot.rows : [];
  ...
  const selectorsByGroup = new Map();
  selectorRows.forEach((selector) => {
    if (!selector?.group_slug) {
      return;
    }
    const slug = selector.group_slug;
    const list = selectorsByGroup.get(slug) ?? [];
    list.push(selector);
    selectorsByGroup.set(slug, list);
  });
  ...
  groupRows.forEach((group) => {
    const selectors = selectorsByGroup.get(group.slug) ?? [];
    ...
    const assetIds = Array.from(matched).sort();
    const existingAssets = Array.isArray(group.assets) ? group.assets : [];
    const assetsMatch =
      assetIds.length === existingAssets.length &&
      assetIds.every((assetId, index) => assetId === existingAssets[index]);

    if (!assetsMatch) {
      store.update('groups', group.slug, { assets: assetIds, updated_at: now });
    }
  });
}
...
export function calculateGroupAssetCoverage() {
  ...
  rows.forEach((row) => {
    ...
    if (assetId && matchedAssets.has(assetId)) {
      return;
    }
    const key = `${assetId ?? ''}:${row?.rowKey ?? ''}:${row?.id ?? ''}`;
    if (unmatchedKeys.has(key)) {
      return;
    }
    unmatchedKeys.add(key);
    const values = buildAssetRowValues(row);
  ...
  return {
    ...
    unmatchedCount: unmatchedAssets.length,
    unmatchedAssets,
    unmatchedColumns: columns,
    groups
  };
}
