...
function listAssetPool({ page = 1, pageSize, includeArchived = false } = {}) {
  const pool = normalizeAssetPool();
  const entries = Object.entries(pool.data).map(([id, data]) => ({ id, ...data }));
  const filtered = entries.filter((entry) => (includeArchived ? true : entry.archived !== true));
  const total = filtered.length;
  const normalizedPage = Number.isInteger(page) && page > 0 ? page : 1;
  const hasPagination = Number.isInteger(pageSize) && pageSize > 0;
  const normalizedPageSize = hasPagination ? pageSize : total;
  const start = hasPagination ? Math.max(0, (normalizedPage - 1) * normalizedPageSize) : 0;
  const limit = hasPagination ? start + normalizedPageSize : filtered.length;
  const rows = normalizedPageSize > 0 ? filtered.slice(start, limit) : [];
...
export function getAssetRowsByIds(ids = [], { includeArchived = false } = {}) {
  ...
  const { rows } = listAssetPool({ includeArchived });
  if (!Array.isArray(rows) || !rows.length) {
    return [];
  }
  const lookup = new Set(normalizedIds);
  return rows.filter((row) => lookup.has(String(row?.id ?? '')));
}
...
export function applyAssetPatches(patches = []) {
  if (!Array.isArray(patches) || !patches.length) {
    return null;
  }
  const pool = normalizeAssetPool();
  const data = pool.data;
  patches.forEach(({ assetId, patch }) => {
    if (!assetId || !data[assetId] || !patch || typeof patch !== 'object') {
      return;
    }
    data[assetId] = { ...data[assetId], ...patch };
  });
  writePool(pool);
  return pool;
}
