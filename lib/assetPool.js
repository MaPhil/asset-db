...
const DEFAULT_ASSET_POOL = { meta: { mapping: {} }, data: {} };
...
export function listAssetPool({ page = 1, pageSize = 50 } = {}) {
  ...
  const allowedFields = mappedFields.length ? mappedFields : getMappedFieldsFromSchema();
  const allowedSet = allowedFields.length ? new Set(allowedFields) : null;
