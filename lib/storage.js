  ...
  try {
    ...
    for (const table of TABLES) {
      ...
      if (!fs.existsSync(filePath)) {
        ...
        logger.info('Initialized storage table', { table });
      }
    }
  } catch (error) {
    logger.error('Failed to initialize storage tables', error);
    throw error;
  }
}
...
function readFile(table) {
  ...
  try {
    ...
    logger.error('Failed to read storage table', error, { table, filePath });
    throw error;
  }
}
...
function writeFile(table, data) {
  ...
  try {
    ...
    logger.error('Failed to write storage table', error, { table, filePath });
    throw error;
  }
}
...
export const store = {
  get(table) {
    logger.debug('Reading from storage table', { table });
    return readFile(table);
  },
  set(table, data) {
    logger.debug('Writing to storage table', { table });
    return writeFile(table, data);
  },
  insert(table, row) {
    logger.debug('Inserting row into storage table', { table });
  ...
  update(table, id, patch) {
    logger.debug('Updating row in storage table', { table, id });
    const data = readFile(table);
    const index = data.rows.findIndex((entry) => entry.id === id);
    if (index === -1) {
      logger.warn('Row not found for update', { table, id });
      return false;
    }
  ...
  remove(table, id) {
    logger.debug('Removing row from storage table', { table, id });
    const data = readFile(table);
    ...
    if (data.rows.length !== originalLength) {
      ...
      logger.debug('No row removed from storage table', { table, id });
    }
  },
  upsertSchemaCol(colName) {
    logger.debug('Upserting schema column', { colName });
    ...
    if (!exists) {
      ...
      logger.debug('Schema column already exists', { colName });
    }
  }
};
