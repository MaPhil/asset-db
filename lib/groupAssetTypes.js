...
function requireGroup(groupId) {
  ...
  if (!Number.isInteger(id) || id <= 0) {
    throw createError('Ungültiger Gruppenbezeichner.', 400);
  }

  const groups = store.get('groups').rows;
  const group = groups.find((row) => row.id === id);
  if (!group) {
    throw createError('Gruppe nicht gefunden.', 404);
  }
  return group;
}
...
export function addAssetTypeToGroup(groupId, name) {
  ...
  if (!normalised) {
    throw createError('Name des Asset-Typs ist erforderlich.');
  }

  const available = getAvailableAssetTypesForGroup(id);
  const match = available.find((entry) => normaliseName(entry.name) === normalised);
  if (!match) {
    throw createError('Asset-Typ ist für diese Gruppe nicht verfügbar.', 409);
  }
  ...
  logger.info('Asset-Typ einer Gruppe zugewiesen', {
    groupId: id,
...
export function removeAssetTypeFromGroup(groupId, assetTypeId) {
  const group = requireGroup(groupId);
  const id = Number(groupId);
  const assignmentId = Number(assetTypeId);
  if (!Number.isInteger(assignmentId) || assignmentId <= 0) {
    throw createError('Ungültiger Bezeichner für die Asset-Typ-Zuweisung.', 400);
  }

  const table = store.get('group_asset_types');
  const row = table.rows.find((entry) => Number(entry?.id) === assignmentId);
  if (!row || Number(row?.group_id) !== id) {
    throw createError('Asset-Typ-Zuweisung für diese Gruppe nicht gefunden.', 404);
  }

  store.remove('group_asset_types', assignmentId);

  const assetTypeName = canonicalName(row?.asset_type);

  logger.info('Asset-Typ aus Gruppe entfernt', {
    groupId: id,
