...
import { getCategoryLocation } from './assetStructure.js';
...
function collectAssetTypeGroupUsage() {
  ...
  function registerUsage(name, groupId) {
    ...
    if (Number.isInteger(categoryId) && categoryId > 0) {
      const location = getCategoryLocation(categoryId);
      if (location?.topic?.id && location?.subTopic?.id) {
        url = `/asset-structure/${location.topic.id}/${location.subTopic.id}/${categoryId}/groups/${group.id}`;
      }
    }
