...
function collectAssetTypeGroupUsage() {
  ...
  groupCategories.forEach((row) => {
    ...
    if (!categoryLookup.has(groupId)) {
      categoryLookup.set(groupId, categoryId);
    }
  });
  ...
  function registerUsage(name, groupId) {
    ...
    const categoryId = categoryLookup.get(id) ?? null;
    const url = Number.isInteger(categoryId) && categoryId > 0
      ? `/asset-structure/categories/${categoryId}/groups/${group.id}`
      : null;
...
export function saveAssetTypeDecision(name, decision, comment) {
  ...
  if (payload.decision === 'ignore') {
    ...
    if (assignments.length > 0) {
      throw createError('Cannot ignore this asset type because it is assigned to one or more groups.', 409);
    }
  }
